...
function updateSpecDoc() {
  ...
  const datePg = body.appendParagraph('最終更新：2026年4月1日');
  ...
  // ── 1. ツール概要 ────────────────────────────────────────────────────
  h1('1. ツール概要');
  p('「次回予約率ツール」は、しんきゅうコンパスに登録する鍼灸院が日々の施術数・次回予約数を記録し、リピート率の推移を把握するためのWebツールです。');
  sp();
  tbl(['項目', '内容'], [
  ...
  tbl(['項目', '仕様'], [
    ...
    ['照合先',       'マスターSSの月次タブ（初回）→ インデックスタブ（2回目以降・高速）'],
    ['内部ログイン', 'settingsタブのmaster_login_emailリストに登録されたアドレスでログイン可'],
    ['セッション',   'localStorageに保持。再訪問時はloginスキップ→getDataのみ（通信1回）'],
  ]);
  sp();
  h2('2-2. パフォーマンス最適化');
  tbl(['最適化', '内容'], [
    ['API統合',       'loginとgetDataを1つのAPI（loginAndGetData）に統合。通信2回→1回'],
    ['インデックス検索', '初回ログイン時にemail列をインデックスに記録。2回目以降は月次タブスキャン不要'],
    ['セッション復元', '再訪問時はgetDataのみ呼び出し（loginスキップ）'],
    ['プログレッシブ表示', 'ログイン押下直後に画面枠を先表示。白画面時間ゼロ'],
  ]);
  sp();
  ...
  tbl(['#', 'セクション名', '内容'], [
    ['①', 'ヘッダー',             '院名・ログアウトボタン'],
    ['②', '今月のリピート率',     '今月累計の大きな数値表示'],
    ['③', '直近月次リピート率',   '直近3ヶ月の月次リピート率をカード表示'],
    ['④', '本日のデータ',         '当日の施術数・次回予約数を入力'],
    ['⑤', 'リピート率グラフ',     '日次・週次・月次の推移グラフ（Chart.js）'],
    ['⑥', '過去データカレンダー', '日付をクリックして過去データを入力・修正'],
  ]);
  sp();
  ...
  h2('4-2. データ入力');
  li('施術数（①）・次回予約数（②）を ±ボタン または テンキー形式で入力');
  li('次回予約数 > 施術数 の場合は保存をブロック');
  li('保存後は「保存済み表示」に切り替わり、「修正する」ボタンで再編集可能');
  li('施術0回で保存 → 既存データがあれば削除、なければ入力を促す');
  sp();

  h2('4-3. 過去データカレンダー');
  li('現在月の日付をカレンダー形式で表示');
  li('各日付セルにリピート率を色分け表示（評価基準に準拠）');
  li('「＜」「＞」ボタンで月移動（グラフも連動）');
  ...
  h2('4-4. リピート率グラフ');
  li('日次（PC表示時のみ）・週次・月次の3モード切り替え');
  li('日次：カレンダーの表示月に連動');
  li('週次：直近8週（月曜起点）のリピート率');
  li('月次：直近6ヶ月のリピート率');
  li('バー上部に「〇%」ラベルを表示するカスタムプラグイン');
  sp();

  h2('4-5. スプレッドシートエクスポート');
  p('GAS API（export）経由で新規Googleスプレッドシートを生成し、新タブで開く。');
  sp();
  tbl(['項目', '仕様'], [
  ...
  tbl(['コンポーネント', '役割'], [
    ...
    ['Google Spreadsheet（マスター）', '院のアカウント管理・インデックス・設定'],
  ...
  sp();
  tbl(['タブ名', '内容'], [
    ['YYMM（月次タブ）',   '有料会員の院情報（サロンID・メール・院名等）。直近3ヶ月分を認証に使用'],
    ['無料体験',           '無料体験院の情報'],
    ['repeat_rate_index',  '院別スプレッドシートID＋email（高速検索用キャッシュ）'],
    ['settings',           'master_login_email リスト（社内ログイン用メール管理）'],
  ]);
  sp();

  h2('5-3. 院別スプレッドシート');
  p('初回ログイン時に自動生成。Google Driveの指定フォルダに保存。');
  sp();
  h3('日次タブ（1行 = 1日分）');
  tbl(['列', '内容'], [
    ['A', '日付（YYYY-MM-DD）'],
  ...
  tbl(['列', '内容'], [
    ...
    ['B（氏名かな）', '名（ひらがな）※現在は空'],
    ['Q（来店日）',  '日付'],
    ['R（番号）',    'レコードID（YYYYMMDD_NNN形式）'],
    ['S（次回予約）', '次回予約の有無（1=あり、0=なし）'],
    ['T（メニュー）', 'メニュー名 ※現在は空'],
  ]);
  p('※ 顧客詳細入力機能は削除済み。保存時は施術数ぶんの空行が記録される。');
  sp();

  h2('5-4. GAS APIエンドポイント');
  h3('POSTエンドポイント（doPost）');
  tbl(['action', '機能'], [
    ['loginAndGetData', 'ログイン認証＋データ取得を1回で完結（推奨）'],
    ['login',           'ログイン認証のみ（後方互換）'],
    ['saveDay',         '日次データの保存（上書き）'],
  ...
  tbl(['対策', '内容'], [
    ...
    ['メタタグ遮断',         'meta robots でGoogle・AI各クローラーを遮断（noindex/nofollow/noarchive）'],
    ['AIクローラー',         'GPTBot（ChatGPT）・CCBot（Claude）等を個別に遮断'],
    ['通信',                 'GitHub PagesはHTTPS配信'],
    ['GAS認証',              'Web AppはANYONE_ANONYMOUSだが、ログイン認証が必須'],
  ...
  tbl(['ファイル', '説明'], [
    ...
    ['state-transition.test.js',  'Layer2 状態遷移テスト（45件）'],
  ...
  tbl(['日付', '内容'], [
    ...
    ['2026-03-23', 'CSVダウンロード・スプレッドシートエクスポート機能追加'],
    ['2026-03-24', '日付切り替え時の残留データ混入バグ修正'],
    ['2026-03-31', '顧客詳細入力機能・CSVダウンロード機能を削除（シンプル化）'],
    ['2026-04-01', 'パフォーマンス最適化（API統合・インデックス高速検索・セッション復元・プログレッシブ表示）'],
  ]);
