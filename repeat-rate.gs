...
function updateSpecDoc() {
  ...
  body.appendParagraph('しんきゅうコンパス｜リピート率改善ツール').setHeading(DocumentApp.ParagraphHeading.SUBTITLE);
  p('作成日：2026年3月24日');
  p('');
  ...
  p('・対象：しんきゅうコンパス登録サロン（院ごとにアカウントが発行される）');
  p('・公開範囲：非公開（robots.txtおよびメタタグでクローラー・AIの収集を全面遮断）');
  p('・アクセス：https://shinq-compass-cs.github.io/repeat-rate/');
  p('');

  // 2. アクセス・ログイン
  h1('2. アクセス・ログイン');
  h2('2-1. ログイン仕様');
  p('・認証方法：サロンID（半角数字）＋ メールアドレス');
  p('・照合先：Google Spreadsheet（マスターシート）の salon_list タブ');
  p('・内部ログイン：info@shinq-compass.jp または test でログイン可（ログ記録スキップ）');
  p('・セッション：ログイン後、サロンデータをメモリに保持（ページリロードで再ログイン）');
  p('');
  h2('2-2. ログイン後の状態');
  p('・ヘッダーに院名を表示');
  p('・当該院の全データを一括ロード（GAS API getData）');
  p('・今月のリピート率を即時計算・表示');
  p('');
  ...
  p('① ヘッダー：院名・ログアウトボタン');
  p('② 今月のリピート率：今月累計の大きな数値表示（評価バッジは非表示）');
  p('③ 本日のデータ：当日の施術数・次回予約数を入力');
  p('④ リピート率グラフ：過去12ヶ月の推移グラフ');
  p('⑤ 過去データカレンダー：日付をクリックして過去データを編集');
  p('');

  // 4. 機能仕様
  h1('4. 機能仕様');
  h2('4-1. リピート率の定義・計算');
  p('・リピート率（%）= 次回予約数 ÷ 施術数 × 100（四捨五入）');
  p('・月次計算：対象月の全日付データを集計');
  p('');
  h3('評価基準');
  p('・90%以上　　　　→ 優秀');
  p('・70%以上 90%未満 → 良好');
  ...
  h2('4-2. 本日のデータ入力');
  p('・施術数（①）・次回予約数（②）をテンキー形式で入力');
  p('・次回予約数 > 施術数 の場合は保存をブロック');
  p('・保存後は「保存済み表示」に切り替わり、「修正」ボタンで再編集可能');
  p('');

  h2('4-3. 患者詳細データ（顧客情報）');
  p('施術数（v）の数だけ患者行が自動生成されます。各行に「詳細▼」ボタンがあり、展開すると以下を入力できます。');
  p('');
  h3('フィールドとバリデーション一覧');
  p('・氏名（漢字）：テキスト / バリデーションなし');
  p('・姓（ひらがな）：【Layer1】ひらがな・長音符のみ入力可');
  p('・名（ひらがな）：【Layer1】ひらがな・長音符のみ入力可');
  p('・メニュー：テキスト / バリデーションなし');
  p('・性別：セレクト（男・女・—）');
  p('・電話番号：【Layer1】数字・ハイフン・括弧のみ入力可');
  p('・メールアドレス：【Layer1】半角英数字・記号のみ / 【Layer2】onblurでx@x.x形式チェック');
  p('・次回予約：チェックボックス');
  p('');
  p('Layer1バリデーション：IMEのcomposing状態をクロージャで管理し、確定後に不正文字をリアルタイム除去。除去時はフィールド直下に注意書きを2秒表示。');
  p('');
  h3('日付切り替え時の安全設計');
  p('別日付のフォームを開く際、前日付の入力値・UIステート（詳細の開閉状態）を完全リセットします。これにより残留データが別日付に混入するバグを防止しています。');
  p('');

  h2('4-4. 過去データカレンダー');
  p('・現在月の日付をカレンダー形式で表示');
  p('・各日付セルにリピート率を色で表示（優秀：ゴールド / 良好：グリーン / 標準：イエロー / 課題あり：レッド / 未入力：空白）');
  p('・「＜」「＞」ボタンで月移動（グラフも連動）');
  p('・日付タップ → 過去フォームに切り替え（カレンダーと相互排他）');
  p('・今日日付はクリック不可（上部「本日のデータ」で入力）');
  p('');

  h2('4-5. CSVダウンロード');
  p('カレンダー下部の「CSVダウンロード」ボタンから実行。');
  p('・内容：当該院のスプレッドシート「顧客」タブをそのまま出力（変換なし）');
  p('・ファイル名：院名_リピート率データ_YYYYMMDD.csv');
  p('・文字コード：BOM付きUTF-8（Excel対応）');
  p('・日付形式：YYYY-MM-DD（Excelでの自動変換を回避）');
  p('・取得方法：GAS APIにfetchしてBlob URL経由でダウンロード（ファイル名制御のため）');
  p('');

  h2('4-6. スプレッドシートで開く');
  p('カレンダー下部の「スプレッドシート」ボタンから実行。');
  p('・ボタン押下のたびに新規Googleスプレッドシートを生成（ファイル名：院名_リピート率データ_YYYYMMDD_HHMMSS）');
  p('・内容：「顧客」タブ・「日次」タブの両方をコピー');
  p('・公開設定：「リンクを知っている全員が閲覧可能」');
  p('・新タブで開く → 「コピーを作成」でユーザー自身のGoogleドライブに保存可能');
  p('');

  // 5. データ管理
  h1('5. データ管理');
  h2('5-1. インフラ構成');
  p('・GitHub Pages：フロントエンドのホスティング（静的HTML/JS/CSS）');
  p('・Google Apps Script：バックエンドAPI（Web App）');
  p('・Google Spreadsheet（マスター）：院のアカウント管理・インデックス');
  p('・Google Spreadsheet（院別）：院ごとの日次・顧客データ');
  p('');

  h2('5-2. マスタースプレッドシート');
  p('ID: 1CiuXRdYG-lI_jWA4DiR_uXvYU9dgqTE1v-JWsZF1l_g');
  p('・salon_list タブ：院のログイン情報（サロンID・メール等）');
  p('・repeat_rate_index タブ：院別スプレッドシートIDの管理テーブル');
  p('');

  h2('5-3. 院別スプレッドシート');
  p('初回ログイン時に自動生成。Google Driveの指定フォルダ（ID: 1hu_VB9WpaKa-Cz-kmrvCV3KeynWlYjIp）に保存。');
  p('');
  h3('日次タブ（1行 = 1日分）');
  p('・A列：日付（YYYY-MM-DD）');
  p('・B列：施術数');
  ...
  p('・login：ログイン認証・院別スプレッドシートの取得または自動生成');
  p('・saveDay：日次データ・顧客データの保存（上書き）');
  p('・getData：当該院の全日次・顧客データを取得');
  p('');
  h3('GETエンドポイント（doGet）');
  p('・csv：顧客タブをCSV形式で返す（BOM付きUTF-8）');
  p('・export：新規Googleスプレッドシートを生成してURLを返す');
  p('');

  h2('5-5. 重複排除ルール');
  p('・同一日付に複数回保存した場合、最後の保存が有効（上書き）');
  p('・日次タブ：同一日付の最終行を残す');
  p('・顧客タブ：同一（日付＋番号）の最終行を残す');
  p('');

  // 6. セキュリティ
  h1('6. セキュリティ・公開範囲');
  p('・robots.txt で全クローラーを遮断');
  p('・<meta name="robots"> でGoogle・AI各クローラーを遮断（noindex/nofollow/noarchive/nosnippet）');
  p('・ChatGPT（GPTBot）、Claude（CCBot）等のAIクローラーも個別に遮断');
  p('・GitHub PagesはHTTPS配信');
  p('・GAS Web AppはCORSを許可（ANYONE_ANONYMOUS）だが、ログインIDと院IDによる認証が必須');
  p('・APIキーは露出なし（GASのマスターSSへのアクセスは認証済みGASプロジェクトのみ）');
  p('');

  // 7. 動作確認環境
  h1('7. 動作確認環境');
  p('・iPhone + Safari（スマートフォン主）');
  p('・Android + Chrome（スマートフォン主）');
  p('・PC + Chrome（補助）');
  p('・ブレークポイント：600px（スマホ/PC切り替え）');
  p('・iOSズーム防止：全テキスト入力フォントサイズ 16px 以上');
  p('');

  // 8. ファイル構成
  h1('8. ファイル構成');
  p('・index.html：フロントエンド本体（HTML/CSS/JS一体型）');
  p('・repeat-rate.gs：GASバックエンド');
  p('・robots.txt：クローラー遮断設定');
  p('・state-transition.test.js：Layer2 状態遷移テスト（46件）');
  p('・gas-api.test.js：GAS統合テスト（39件）');
  p('');

  // 9. 更新履歴
  h1('9. 更新履歴');
  p('・2026-02：初版リリース');
  p('・2026-03：顧客詳細フォーム・メニュー列追加');
  p('・2026-03-20：バリデーション実装（Layer1入力制限・Layer2形式検証）');
  p('・2026-03-23：CSVダウンロード機能追加（顧客タブそのまま出力・院名付きファイル名）');
  p('・2026-03-24：スプレッドシートエクスポート機能追加（毎回新規生成）');
  p('・2026-03-24：日付切り替え時の残留データ混入バグ修正');
